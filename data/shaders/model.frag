...
void main()
{
    ...
    if (useBumpMap)
    {
        float A = textureOffset(bumpMap, uv, ivec2( 1, 0)).x;
        float B = textureOffset(bumpMap, uv, ivec2(-1, 0)).x;
        float C = textureOffset(bumpMap, uv, ivec2( 0, 1)).x;
        float D = textureOffset(bumpMap, uv, ivec2( 0,-1)).x;
